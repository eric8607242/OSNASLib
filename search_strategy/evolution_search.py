...
def evoluation_algorithm(trainer, training_strategy, supernet, val_loader, lookup_table, target_hc, logger, generation_num=20, population=60, parent_num=30, info_metric="flops"):
    ...
    population_fitness = get_population_accuracy(new_population, trainer, supernet, val_loader, info_metric)
    ...
    for g in range(generation_num):
        logger.info("Generation : {}, Time : {}".format(g, time.time()-start_time))
        cur_best_fitness = np.max(population_fitness)

        if global_best_fitness < cur_best_fitness:
            global_best_fitness = cur_best_fitness
            logger.info("New global best fitness : {}".format(global_best_fitness))

        parents, parents_fitness = select_mating_pool(new_population, population_fitness, parent_num)
        ...
        while evoluation_id < offspring_size:
            ...
            offspring_evolution = mutation(offspring_evolution, training_strategy)

            offspring_hc = lookup_table.get_model_info(offspring_evolution, info_metric=info_metric)
        ...
        offspring_fittness = get_population_accuracy(offspring_evolution, trainer, supernet, val_loader, info_metric)
...
def get_population_accuracy(population, trainer, supernet, val_loader, info_metric="flops"):

...
from utils import get_optimizer, min_max_normalize, save
...
class ArchitectureGeneratorSearcher(BaseSearcher):
    def __init__(self, supernet, val_loader, lookup_table, training_strategy, device, logger):
        ...
        self.g_optimizer = get_optimizer(generator.parameters(),
        ...
        arch_param_nums = self.supernet.get_arch_param_nums()
        self.prior_pool = PriorPool(self.lookup_table, arch_param_nums, self.config)
    ...
    def search(self):
        ...
        for epoch in range(search_epochs):
            ...
            self._generator_training_step(tau)
            ...
            top1_avg = self._generator_validate(target_hardware_constraint=target_val_hardware_constraint)
            ...
            if total_loss < best_loss:
                ...
                save(self.generator, self.config["experiment_path"]["best_loss_generator_checkpoint_path"], self.g_optimizer, epoch+1)
                best_loss = total_loss

            if best_top1 < top1_avg:
                self.logger.info(f"The newest best top1 accuracy achieve {top1_avg}! Save model.")
                save(self.generator, self.config["experiment_path"]["best_acc_generator_checkpoint_path"], self.g_optimizer, epoch+1)
                best_top1 = top1_avg

            tau *= self.config["generator"]["tau_decay"]


    def _generator_training_step(self, tau):
    ...
    def _generator_validate(self, target_hardware_constraint):
        with torch.no_grad():
            for step, (X, y) in enumerate(self.val_loader):
    ...
    def _intermediate_stats_logging(self, outs, y, ce_loss, hc_loss, step, epoch, N, len_loader, val_or_train, print_freq=100):
        ...
        if (step > 1 and step % print_freq == 0) or step == len_loader - 1:
            self.logger.info(f"{} : [{:3d}/{}] Step {:03d}/{:03d} Loss {:.3f} CE Loss {:.3f} HC Loss {:.3f} Prec@(1, 5) ({:.1%}, {:.1%})" .format(
                    val_or_train,
                    epoch + 1,
                    self.epochs,
                    step,
